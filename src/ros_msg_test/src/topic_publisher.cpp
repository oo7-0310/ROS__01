...

{
    ...
    // 퍼블리셔 선언,  ros_msg_test  패키지의 MsgTest 메시지 파일을 이용한

    // 퍼블리셔  topic_publisher  를 작성한다. 토픽명은 "ros_test_msg" 이며,

    // 퍼블리셔 큐(queue) 사이즈를 100개로 설정한다는 것이다

    ros::Publisher topic_publisher = nh.advertise< ros_msg_test :: MsgTest >("ros_test_msg", 100);
    ...
    //  MsgTest  메시지 파일 형식으로 msg 라는 메시지를 선언

     ros_msg_test :: MsgTest msg;    
...
// 메시지에 사용될 변수 선언
...

    {
        ...
        ROS_INFO("send msg = %d", msg.data);    // data 메시지를 표시한다
         ...
         topic_publisher.publish(msg);    // 메시지를 발행한다
